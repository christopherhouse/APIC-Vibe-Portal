...
class TestCosmosPersistence:
    """Tests that verify chat sessions are persisted to Cosmos DB."""

    @pytest.fixture
    def mock_repo(self):
        """Return a mock ChatSessionRepository."""
        repo = MagicMock()
        repo.find_by_id.return_value = None
        repo.create.return_value = {"id": "test-session"}
        repo.update.return_value = {"id": "test-session"}
        repo.soft_delete.return_value = {"id": "test-session", "isDeleted": True}
        return repo

    @pytest.fixture
    def service_with_repo(self, mock_agent_router, mock_repo):
        """Return an AIChatService with a mock chat repository."""
        return AIChatService(
            agent_router=mock_agent_router,
    ...
    @pytest.mark.asyncio
    async def test_chat_creates_session_in_cosmos(self, service_with_repo, mock_repo):
        """First chat message should create a new document in Cosmos DB."""
        await service_with_repo.chat("Hello", session_id="new-sess", user_id="user-123")
        mock_repo.find_by_id.assert_called()
        mock_repo.create.assert_called_once()
        created_doc = mock_repo.create.call_args.args[0]
        assert created_doc["userId"] == "user-123"
        assert created_doc["id"] == "new-sess"
        assert len(created_doc["messages"]) == 2  # user + assistant

    @pytest.mark.asyncio
    async def test_chat_updates_existing_session(self, service_with_repo, mock_repo):
        """Subsequent messages should update the existing document."""
        ...
        await service_with_repo.chat("Follow up", session_id="existing-sess", user_id="user-123")
        mock_repo.update.assert_called_once()
        updated_doc = mock_repo.update.call_args.args[0]
        assert len(updated_doc["messages"]) == 2

    @pytest.mark.asyncio
    async def test_chat_without_user_id_skips_persistence(self, service_with_repo, mock_repo):
        """When user_id is None, no Cosmos write should happen."""
        await service_with_repo.chat("Hello", session_id="no-user")
        mock_repo.find_by_id.assert_not_called()
        mock_repo.create.assert_not_called()

    @pytest.mark.asyncio
    async def test_chat_without_repo_skips_persistence(self, mock_agent_router):
        """When chat_repository is None, no Cosmos write should happen."""
        svc = AIChatService(agent_router=mock_agent_router)
        result = await svc.chat("Hello", session_id="no-repo", user_id="user-123")
        assert result.session_id == "no-repo"

    @pytest.mark.asyncio
    async def test_stream_persists_to_cosmos(self, service_with_repo, mock_repo, mock_agent_router):
        """Streaming chat should also persist to Cosmos DB."""
        events = [
            e async for e in service_with_repo.chat_stream("Hello", session_id="stream-persist", user_id="user-456")
        ]
        mock_repo.create.assert_called_once()
        created_doc = mock_repo.create.call_args.args[0]
        assert created_doc["userId"] == "user-456"
        assert len(events) >= 2  # start + content/end events

    @pytest.mark.asyncio
    async def test_persistence_failure_does_not_break_chat(self, service_with_repo, mock_repo):
        """Cosmos DB write failures should be logged but not propagated."""
        mock_repo.find_by_id.side_effect = Exception("Cosmos DB unavailable")
        result = await service_with_repo.chat("Hello", session_id="fail-sess", user_id="user-789")
        assert result.message.content == "Agent says: here are your APIs."

    def test_get_history_falls_back_to_cosmos(self, service_with_repo, mock_repo):
        """get_history should fall back to Cosmos when in-memory is empty."""
        mock_repo.find_by_id.return_value = {
            "id": "cosmos-sess",
            "userId": "user-123",
            "messages": [
        ...
        messages = service_with_repo.get_history("cosmos-sess", user_id="user-123")
        assert len(messages) == 2
        assert messages[0].role == "user"
        assert messages[1].role == "assistant"
        mock_repo.find_by_id.assert_called_once_with("cosmos-sess", "user-123")

    def test_clear_history_soft_deletes_from_cosmos(self, service_with_repo, mock_repo):
        """clear_history should soft-delete from Cosmos DB."""
        service_with_repo.clear_history("del-sess", user_id="user-123")
        mock_repo.soft_delete.assert_called_once_with("del-sess", "user-123")

    def test_clear_history_without_user_id_skips_cosmos(self, service_with_repo, mock_repo):
        """clear_history without user_id should not touch Cosmos."""
        service_with_repo.clear_history("del-sess")
