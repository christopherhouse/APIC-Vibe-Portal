"""Azure Monitor OpenTelemetry distro initialization for the indexer.

Call ``configure_telemetry()`` once at the start of the indexer job to
wire up distributed tracing, metrics, and log export to Application
Insights via a single ``configure_azure_monitor()`` call.
...
_SERVICE_NAME = "apic-vibe-portal-indexer"
_SERVICE_VERSION = "0.0.0"


def configure_telemetry(*, connection_string: str | None = None) -> None:
    """Initialize the Azure Monitor OpenTelemetry distro for the indexer job.
    ...
    try:
        ...
        logger.info("Azure Monitor OpenTelemetry distro configured for indexer")
    except ImportError:
