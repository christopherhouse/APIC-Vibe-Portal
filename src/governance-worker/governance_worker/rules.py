"""Governance rules and compliance checker for the governance snapshot worker.

This module is a self-contained copy of the rule engine from the BFF
(``apic_vibe_portal_bff.agents.governance_agent.rules``).  It is
duplicated here so that the governance-worker Container Job has no
dependency on the BFF package, keeping container images small and
deployment boundaries clean.

Any changes to the rule set should be kept in sync with the BFF copy.
"""
