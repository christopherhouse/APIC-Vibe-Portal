...
| 2026-04-30 | ✅ Complete    | GitHub Copilot agent | Reviewed and remediated under [task 035](035-api-center-backup-remediation.md) — fifteen findings (5 critical, 5 medium, 5 minor) addressed including CI/CD wiring of the backup-job container, pagination contract, failed-backup audit trail, Cosmos custom RBAC role scoped to the backup-metadata container, streaming ZIP via tempfile + 2 GiB memory, BFF blob role reduced to `Storage Blob Delegator`, partition-keyed list query, `BackupSummary` model strictness, smarter spec-extension detection, layout-level admin guard, and lifecycle-policy runbook. |
...
- **Reviewed and remediated under [task 035](035-api-center-backup-remediation.md)** — see that document for the fifteen findings and their resolutions (CI/CD wiring, pagination contract, failed-backup audit trail, Cosmos custom RBAC role, streaming ZIP, BFF role narrowing, partition-keyed query, model strictness, spec-extension heuristic, ID granularity, error-class narrowing, lifecycle-rule runbook, layout-level admin gate).
